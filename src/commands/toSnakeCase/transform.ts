...
function transformObject(data: Record<string, any>) {
  ...
  keys.forEach((key) => {
    ...
    if (valType === "object" && Array.isArray(val)) {
      ...
      temp[snakeCase(key)].properties = tpl.properties;
      temp[snakeCase(key)].elementType = tpl.elementType;
      if (isBaseType(tpl.elementType)) {
        delete temp[snakeCase(key)].properties;
      }
      return;
    }
    temp[snakeCase(key)].properties = transformObject(val);
    temp[snakeCase(key)].elementType = "object";
  });
